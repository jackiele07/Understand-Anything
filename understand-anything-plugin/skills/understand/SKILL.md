...
## Phase 5.5 — SEQUENCE DIAGRAMS (only runs when `--with-flowchart` in `$ARGUMENTS`)

Generate Mermaid sequence diagrams for all detected API entry points. This phase is self-contained — it reads `knowledge-graph.json` (or `assembled-graph.json` for full analysis) directly.

### Step 1 — Detect entry points

Scan for all API entry points using three strategies in order:

**Strategy A: `endpoint:` nodes in the graph**
Search `assembled-graph.json` (or existing `knowledge-graph.json` for fast-path) for all nodes with `"type": "endpoint"`. Each is an entry point.

**Strategy B: Source file fallback (when Strategy A yields 0 results)**
Scan the project source directly for these patterns:
- **REST** — find `*Controller.cs` files, grep for `[HttpGet`, `[HttpPost`, `[HttpPut`, `[HttpDelete`, `[HttpPatch`, `[Route(` attributes. Extract method + path per attribute.
- **gRPC** — find `*.proto` files with `service` blocks, extract each `rpc` definition as an entry point.
- **WebSocket** — find `Program.cs` / `Startup.cs` files, grep for `.MapHub(`, `.Map("/ws`, `UseWebSockets`. Extract path.
- **NATS subscriptions** — grep entire codebase for `Subscribe(`, `SubscribeAsync(`, `AddHandler(`, topic handler registrations. Extract topic name.

**Strategy C: Hybrid** — use Strategy A nodes AND augment with Strategy B results for any protocols not covered by Strategy A nodes.

For each detected entry point produce:
```json
{
  "id": "seq:<file>:<method-or-name>",
  "title": "<HTTP METHOD /path> or <topic: name> or <rpc: ServiceName.Method>",
  "protocol": "REST" | "gRPC" | "WebSocket" | "NATS",
  "entryFile": "<controller or handler file path>",
  "entryNodeId": "<endpoint: node id if exists, else null>"
}
```

### Step 2 — Generate sequence diagrams (LLM)

For each entry point, generate a Mermaid `sequenceDiagram` by:

1. Reading the relevant node summaries and edges from the graph for:
   - The entry point's file and its direct dependencies (1-hop)
   - Any files with `calls`, `depends_on`, `publishes`, `subscribes` edges reachable from the entry point
   - Cross-process boundaries: subprocess spawns, NATS publish/subscribe, WebSocket connections
2. Tracing the **full cross-process call chain** end-to-end, including:
   - In-process: controller → service → repository/handler
   - Subprocess boundary: note as `Note over X: spawns subprocess`
   - Message queue: show publish → broker → subscriber as separate participants
   - WebSocket: show as bidirectional arrow `Device->>Gateway: MemoryPack request`
3. Labeling async boundaries explicitly: `Note over A,B: async / NATS CloudEvent`

Produce valid Mermaid `sequenceDiagram` syntax. Participants should use short component names (e.g. `Client`, `InvestigationController`, `InvestigationHandler`, `Runner`, `NATS`, `ToolRequestHandler`, `DeviceConnection`, `Device`).

**Validation**: after generating each diagram, verify basic Mermaid syntax:
- Must start with `sequenceDiagram`
- All `->>`, `-->>`, `-x`, `--x` arrows must have valid participant names on both sides
- `Note over` must reference valid participants
- No unmatched `loop`/`alt`/`opt` blocks

If a diagram fails validation, regenerate it once with the validation error as feedback.

### Step 3 — Save diagrams

Produce a `diagrams` array:
```json
[
  {
    "id": "seq:Investigator/InvestigationController.cs:POST-api-investigations",
    "title": "POST /api/investigations",
    "protocol": "REST",
    "entryPoint": "endpoint:Investigator/InvestigationController.cs:POST-api-investigations",
    "mermaid": "sequenceDiagram\n  participant Client\n  ..."
  }
]
```

**For fast-path** (no full analysis): read existing `knowledge-graph.json`, add/replace the `diagrams` key, write back to `knowledge-graph.json`.

**For full analysis path**: store `diagrams` array in memory; Phase 6 will include it in the assembled graph.
